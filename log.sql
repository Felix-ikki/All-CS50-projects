...
WHERE day = 28 AND month = 7 AND year = 2020 AND street = "Chamberlin Streets"

-- Work with the descrption
SELECT transcript
FROM interviews
WHERE day = 28 AND month = 7 AND year = 2020 AND street = "&courthouse&"
...
WHERE day = "28" AND month = "7" AND year = "2020" AND transaction_type = "withdraw" AND atn_loctaion = "Fifer Street"

-- Working with the third transcript:
-- "As the thief was leaving the courthouse, they called someone who
-- talked to them for less than a minute. In the call, I heard the thief
-- say that they were planning to take the earliest flight out of
-- Fiftyville tomorrow. The thief then asked the person on the other end
-- of the phone to purchase the flight ticket. First check the
...
SELECT DISTINCT naMe
FROM people
JOIN phone_calls ON people.phone_number = phone _calls.caller
